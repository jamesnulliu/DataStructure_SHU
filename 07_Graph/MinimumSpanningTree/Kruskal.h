...
auto createEdgeMinHeap(const Graph_AM<VertTy, WeightTy, UNLINK>& graph) -> std::deque<KruskalEdge<VertTy, WeightTy>>
{
    sizet vertNum = graph.get_vertNum();                   // Number of vertexes in {graph}
    std::deque<KruskalEdge<VertTy, WeightTy>> edgeMinHeap;  // A queue of edges to store KruskalEdges
        ...
        for (index c = 0; c < r; ++c) {
            ...
            if (graph.get_weight_loc(r, c) != UNLINK) {
                ...
                edgeMinHeap.push_back(
                    KruskalEdge<VertTy, WeightTy>{ graph._vertVec[r], graph._vertVec[c], graph.get_weight_loc(r, c) }
                );
            }
        }
    }
    ...
    std::ranges::make_heap(edgeMinHeap, std::greater<>{}, & KruskalEdge<VertTy, WeightTy>::weight);
...
{
    std::vector<std::set<VertTy>> setVec{};  // A vector contains all different sets
    for (auto& vert : graph._vertVec) {
        setVec.push_back(std::set<VertTy>{vert});
...
template <class VertTy, class WeightTy, WeightTy UNLINK>
auto minSpanTree_Kruskal(const Graph_AM<VertTy, WeightTy, UNLINK>& graph) -> std::vector<std::set<VertTy>>
{
    sizet vertNum = graph.get_vertNum();  // Number of vertexes in {graph}

    std::deque<KruskalEdge<VertTy, WeightTy>>
    ...
    while (nodeNum < vertNum - 1) {
        ...
        KruskalEdge<VertTy, WeightTy> krusalEdge = edgeMinHeap.front();

        // Pop out the front edge:
        std::ranges::pop_heap(edgeMinHeap, std::greater<>{}, & KruskalEdge<VertTy, WeightTy>::weight);
        ...
        if (setIterPair.first != setIterPair.second) {
            ...
            if (setIterPair.first - setIterPair.second > 0) {
                *(setIterPair.second) = std::set<VertTy>(outSet.begin(), outSet.end());
                setVec.erase(setIterPair.first);
            } else {
                *(setIterPair.first) = std::set<VertTy>(outSet.begin(), outSet.end());
